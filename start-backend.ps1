...
if ($springbootDir) {
    ...
    echo "Starting Spring Boot on http://127.0.0.1:8091 (Using Local $env:DB_HOST MySQL)"
    $mavenArgs = @(
        "spring-boot:run",
        "-Dspring-boot.run.arguments=--server.port=8091 --spring.profiles.active=dev --app.mock-data.enabled=true --app.public-base-url=https://cpuzhbc.cn --spring.datasource.url=jdbc:mysql://localhost:3306/dachuang?useSSL=false&serverTimezone=UTC&allowPublicKeyRetrieval=true --spring.datasource.username=root --spring.datasource.password= --spring.flyway.enabled=true"
    )
