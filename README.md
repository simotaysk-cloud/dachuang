...
4. **运行服务**：
   ...
   ```
   服务默认运行在 `http://127.0.0.1:8091`（避免与本地已有服务占用 `8081` 冲突）。

### 2. 前端部署 (微信小程序)

1. **导入项目**：使用微信开发者工具打开 `前端代码/miniprogram-5` 文件夹。
2. **修改配置**：
   - 打开 `前端代码/miniprogram-5/utils/config.js`，确认 `apiBaseUrl` 指向您的后端地址（上架建议使用 `https` 公网域名）。
   - 也可在小程序“首页 / 登录页 -> 后端URL配置”中动态修改并测试连通性。
   - 在开发者工具中勾选“详情 -> 本地设置 -> 不校验合法域名、web-view（业务域名）、TLS版本以及HTTPS证书”。
3. **运行预览**：点击“编译”即可查看效果。

## 📦 小程序上架前清单（必做）

1. 后端服务切换为公网 **HTTPS** 域名（不要使用 `127.0.0.1`/`192.168.x.x`）。
2. 微信公众平台配置“服务器域名”（request/uploadFile/downloadFile/socket）。
3. 完成小程序类目、隐私政策、备案等合规信息。
4. 使用微信开发者工具上传版本，先设为体验版，再提交审核。
5. 审核通过后发布线上版本。
